...
Neural mod açıkken proxy, erken `LoadLibraryA/W` ve `GetProcAddress` çağrılarını izler; eksik `nvngx.dll`, `nvngx_dlss.dll` ve `sl.dlss.dll` istekleri için kontrollü sanal modül/export katmanı sağlar. `sl.interposer.dll` ve `sl.common.dll` çekirdek Streamline modülleri çok sayıda zorunlu export içerdiğinden sahte handle ile değiştirilmez, ancak yükleme denemeleri loglanır. Oyun bu isimleri hiç aramıyorsa menü görünürlüğü daha erken bir oyun/adapter kontrolü tarafından engellenmektedir.
...
With neural mode enabled, the proxy observes early `LoadLibraryA/W` and `GetProcAddress` calls and provides a controlled virtual module/export layer for missing `nvngx.dll`, `nvngx_dlss.dll`, and `sl.dlss.dll` requests. Core `sl.interposer.dll` and `sl.common.dll` modules require many exports and are therefore logged but never replaced with a fake handle. If the game never requests these names, an earlier game/adapter check is still suppressing the menu path.
